-- back compat for old kwarg name
    ...
    merge into `oss-big-query-dashboard-prod`.`mart`.`mrt_meta_basic_campaign_stats` as DBT_INTERNAL_DEST
        using (
        select
        * from `oss-big-query-dashboard-prod`.`mart`.`mrt_meta_basic_campaign_stats__dbt_tmp`
